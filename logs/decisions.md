...
- `src/evaluation/metrics.py`: thin wrappers around `epftoolbox.evaluation`
  ...
  the published Lago et al. benchmark. No plain MAPE exposed (PROJECT_SPEC.md —
...
- `src/evaluation/walk_forward.py`: rolling-origin (walk-forward) split
  ...
  364`, `optuna.n_trials: 50` (per PROJECT_SPEC.md), `random_seed: 42`.
...
- `src/models/sarimax.py`: 24 independent per-hour `statsmodels` SARIMAX
  ...
  **Logged deviation from strict daily recalibration**: fully refitting
  ...
  refits. Disclosed here rather than silently baked in, since PROJECT_SPEC.md's
...
- `src/evaluation/run_baselines.py` (new): wires
  ...
  table export (model x target x metric, per the results-export
  conventions note) is a separate, not-yet-built task.
...
- Ensemble machinery (`src/evaluation/ensemble.py`), in the week-7
  priority order from PROJECT_SPEC.md: static weighted combiner + weight
...
- **The `v1.0-results` tag slips past the end-of-week-7 target in
  PROJECT_SPEC.md.** An audit of what the tag must cover found two required
...
`scripts/run_ensemble.py`, `tests/test_ensemble.py`, and PROJECT_SPEC.md.
...
guard PROJECT_SPEC.md calls non-negotiable was silently not running**, exactly where
...
to the Lago benchmark. PROJECT_SPEC.md's data rule is not about protocol
...
**What ran.** The interrupted 2026-08-25 repository-map build was resumed and
...
against the uncached-file list found the chunks covered only 41 of 48
...
`repo-map/` (`graph.html`, `GRAPH_REPORT.md`, `graph.json`). Extraction
...
Read-only with respect to the thesis: nothing outside `repo-map/` was
modified, no frozen artifact was touched, and `git status` showed only the
untracked map-ignore file.
...
**Correction carried out of this session.** `PROJECT_SPEC.md` records the regime
...
CHECKLIST.md already carries for section 3-8. The PROJECT_SPEC.md figure is the
stale one; use 62.6989 when writing. No result is affected — the calm/stressed
split is 651/77 either way.

**Standing instruction added.** `PROJECT_SPEC.md`'s session-startup section now
requires consulting the repository map before grepping cold, and re-establishing
...
in PROJECT_SPEC.md forbids. They are implemented as stubs that raise
...
**ENTSO-E permission, per user decision.** PROJECT_SPEC.md's "no registration-gated
...
absent from the model registry, imported by nothing in the pipeline. PROJECT_SPEC.md's
